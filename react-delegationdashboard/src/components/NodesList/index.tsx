...
const NodesTable = () => {
    ...
    const setQueuedKeys = async (queued: any, adaptedNodesStatus: NodeType[]) => {
        if (queued.length) {
            const results = await Promise.all(
                ...
                ...queued.map((blsKey: any) => getQueueIndex(blsKey, dapp, stakingContract, auctionContract))]
            );
    ...
    const getDiplayNodes = () => {
        Promise.all([getAllNodesStatus(dapp, delegationContract), 
            getBlsKeysStatus(dapp, queued, delegationContract, auctionContract)]).then(
